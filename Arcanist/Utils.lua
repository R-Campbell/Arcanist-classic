...
function Arcanist.Utils.Contains(list, item)  -- must simple list!
...
function Arcanist.Utils.GetItemInfo(itemID) -- Get item info, return only what we want
...
function Arcanist.Utils.GetItemName(itemID)
	local name = Arcanist.Utils.GetItemInfo(itemID)
	return name
end

function Arcanist.Utils.GetSpellName(spell_id) -- Get spell info, return only what we want
...
--]]
function Arcanist.Utils.GetSpellCooldown(id, place)
...
function Arcanist.Utils.GetBagName(container)
	...
	then
		...
		if itemName then
			Arcanist.Utils.ParseItemLink(itemLink)
		else
...
function Arcanist.Utils.ParseItemLink(itemLink)
	local r_id = nil
	local r_name = ""
...
function Arcanist.Utils.printable(tb, level)
  level = level or 1
...
function Arcanist.Utils.TimeLeft(sec)
	local seconde = sec
	local display, minute, second
	if seconde <= 59 then
		display = ":"..tostring(floor(seconde))
	else
